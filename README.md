...
Existing `--bin-dir` paths now also fail closed when they point at a file, a
broken symlink, or a path nested beneath either one instead of a directory, so
status/install/uninstall do not silently treat a non-directory path like an
empty bin.
...
summary so empty, healthy, partial, unmanaged, and mixed states are obvious
before the per-command rows. Legacy installer-managed wrappers now count as
healthy for that summary instead of being lumped into the mixed state.
